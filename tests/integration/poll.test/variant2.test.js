...
describe(`Poll API - Variant 2: ${pollTwoBody.title}`, () => {
  ...
  test('User 1 checks responses after poll threshold is reached', async () => {
    ...
    expect(resp.body[2].owner).toBe(admin.username)
    expect(resp.body[2].choice).toBe(CHOICE1_TEXT)
  })
  ...
  test('Registered user checks responses after poll is closed', async () => {
    const resp = await request(app)
      .get(`${BASE_API}/${pollId}/responses`)
